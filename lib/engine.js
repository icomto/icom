	...
	if(d.e) {
		for(var e in d.e){
			...
			if(typeof d.e[e] == 'object'){
				switch(d.e[e].fn) {
				case 1:
					for(var i = 0; i < d.e[e].d.length; i++) $('#' + e).append(d.e[e].d[i]);
					break;
				case 2:
					for(var i = 0; i < d.e[e].d.length; i++) $('#' + e).prepend(d.e[e].d[i]);
					break;
				case 3:
					for(var i = 0; i < d.e[e].d.length; i++) $('#' + e).before(d.e[e].d[i]);
					break;
				case 4:
					for(var i = 0; i < d.e[e].d.length; i++) $('#' + e).after(d.e[e].d[i]);
					break;
				}
			}
			else if(e == '__obj__') {
...
var iC = function(obj, data, args, useHistory, successCb) {
	...
	opts.href = '/'+ilrtd.current_language + '/ajax/' + String(opts.href).replace(/^https?:\/\/[^\/]+/i, '').replace(/^\/(de|en)\//, '/');
...
function initAjax() {
	...
	$('a:not(.lb-anc):not([target]):not([name]):not([onclick]):not([rel])[href!=""]')
		.addClass('lb-anc')
		.click(function(){
