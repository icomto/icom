...
/* new image system
 * just an idea, not ready now

CREATE TABLE img_files (
	img_id BIGINT NOT NULL PRIMARY KEY,
...
class img {
	...
	public static function insert($data, $name, $force_handling, &$error = NULL) {
		...
		$update = "bullshit=$rand, ext='".db()->escape_string($ext)."', ext_thumb='".db()->escape_string($ext_thumb)."', name='$name', size='$size', width=$width, height=$height, uploader='', user_id='".(IS_LOGGED_IN ? USER_ID : 0)."'";
		$query = "
			INSERT INTO images
			SET id='$id', $update
