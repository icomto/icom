...
if(!in_array(@$_GET['id'], array('register', 'report_page', 'cookie_spammer', 'cookie_spammer_img'))) die;
...
default:
	...
	session::$s['captcha_'.$_GET['id']] = $captcha->CreateImage();
	session()->save();
	break;
...
case 'cookie_spammer':
	...
	cache_L2::set('cookie_'.$_GET['id'].'_img_'.$text, 5*60, 1);
	$text = captcha_encrypt($text);
	echo 'document.write(\'<input type="hidden" name="captcha_text" value="'.$text.'"><img src="/de/captcha/'.$_GET['id'].'_img?text='.$text.'">\');';
	break;
	
case 'cookie_spammer_img':
	$text = captcha_decrypt(@$_GET['text']);
	if(cache_L2::get('cookie_'.$_GET['id'].'_'.$text) === false) return;
...
class SimpleCaptcha {
    ...
    public function CreateImage($text = NULL) {
        ...
        /** Text insertion */
        if($text === NULL) $text = $this->GetCaptchaText();
